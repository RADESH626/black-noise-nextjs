...
function DesignRealClothesSection({ imgSrc, title, description, imageLeft = false }) {
  // Delays distintos dependiendo de si la imagen va a la izquierda o a la derecha
  const baseDelay = 0.4;                    // cuando aparece el primer <h1>
  const imgDelay  = imageLeft ? 0.2 : 0.8;  // la imagen entra antes si va a la izquierda
  ...
  return (
    ...
    >
      <div
        className={`flex flex-col md:flex-row ${imageLeft ? 'md:flex-row-reverse' : ''} justify-between items-center gap-4 w-full max-w-7xl`}
      >
        {/* Texto animado */}
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.6, delay: imgDelay, ease: 'easeOut' }}
